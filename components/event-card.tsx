...
export function EventCard({ event }: { event: AcademyEvent }) {
  ...
  return (
    <article className={cn("rounded-3xl border p-5 transition", urgent ? "border-amber-300/40 bg-amber-300/[.12]" : "border-academy-line/10 bg-academy-card/[.055] hover:border-academy-blue/40")}>
      <div className="flex flex-wrap items-center gap-2">
        <Badge tone={urgent ? "amber" : "blue"}>{event.eventType}</Badge>
        {urgent && <Badge tone="red">Check before class</Badge>}
      </div>
      <h3 className="mt-4 text-2xl font-black leading-tight text-academy-foreground">{event.title}</h3>
      <div className="mt-4 space-y-2 text-sm font-bold text-academy-muted">
